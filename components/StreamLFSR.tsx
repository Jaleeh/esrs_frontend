...
export default function StreamLFSR({ active, type }: VigProps) {
    ...
    if (encryptText && encryptkey) {
      try {
        const response = await fetch(
          `http://cypherlogic.co.uk:8080/crypto-0.1/stream/encrypt/${encryptText}/${encryptkey}`
        );
  ...
  const DecryptMe = async () => {
    if (decrypt && encryptkey) {
      try {
        const response = await fetch(
          `http://cypherlogic.co.uk:8080/crypto-0.1/stream/decrypt/${decrypt.ciphertext}/${encryptkey}`
        );
        const decrypt1 = await response.json();
        ...
        <div className="m-4 items-center flex justify-center">
          <div className="flex space-x-4 items-center">
            ...
            <div className="flex-col">
              <div>
                <label className="font-momcakeBold"> Vector </label>
              </div>
              <textarea
                onChange={(e) =>
              ...
              />
              <div>
                <label className="font-momcakeBold"> Taps </label>
              </div>
              <input
                onChange={(e) => settaps(e.target.value.replace(/ /g, ""))}
                className="bg-inherit border-2 border-blue-500 text-left  font-momcakeThin items-center pl-2 pr-2 outline-1"
                type="text"
                name=""
                id=""
