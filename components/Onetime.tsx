...
export default function Onetime({ active, type }: VigProps) {
    ...
    if (encryptText && encryptkey) {
      try {
        const response = await fetch(
          `http://cypherlogic.co.uk:8080/crypto-0.1/vigenere/otp/${encryptText}/${encryptkey}`
        );
  ...
  const DecryptMe = async () => {
    if (decrypt && encryptkey) {
      try {
        const response = await fetch(
          `http://cypherlogic.co.uk:8080/crypto-0.1/vigenere/otp/${decrypt.ciphertext}/${encryptkey}`
        );
        const decrypt1 = await response.json();
