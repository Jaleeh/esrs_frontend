...
export default function Scytale({ active, type }: VigProps) {
    ...
    if (encryptText && encryptkey) {
      try {
        const response = await fetch(
          `http://cypherlogic.co.uk:8080/crypto-0.1/scytale/encrypt/${encryptText}/${encryptkey}`
        );
  ...
  const DecryptMe = async () => {
    if (decrypt && encryptkey) {
      try {
        const response = await fetch(
          `http://cypherlogic.co.uk:8080/crypto-0.1/scytale/decrypt/${decrypt.ciphertext}/${encryptkey}`
        );
        const decrypt1 = await response.json();
