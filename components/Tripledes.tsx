...
export default function Tripledes({ active, type }: VigProps) {
    ...
    if (encryptText && encryptkey) {
      try {
        const response = await fetch(
          `http://cypherlogic.co.uk:8080/crypto-0.1/3des/encrypt/${encryptText}/${encryptkey}`
        );
  ...
  const DecryptMe = async () => {
    if (decrypt && encryptkey) {
      try {
        const response = await fetch(
          `http://cypherlogic.co.uk:8080/crypto-0.1/3des/decrypt/${decrypt.ciphertext}/${encryptkey}`
        );
        const decrypt1 = await response.json();
    ...
    return (
      <div>
        <div className="text-center font-momcakeBold text-lg">
          <p>Triple Data Encryption Standard</p>
        </div>
        <div className="m-4 items-center flex justify-center">
          <div className="flex space-x-4 items-center">
            <div>
              <div>
                <label className="font-momcakeBold">Plaintext in hex</label>
              </div>
              <div className="">
                <input
                  onChange={(e) =>
            ...
            <div className="flex-col">
              <div>
                <label className="font-momcakeBold"> Key in hex </label>
              </div>
              <input
                onChange={(e) =>
